"""
Example 6b: Optimizing rotation of a teapot with the soft mesh renderer.

This example converges for small perturbations in rotation but not larger perturbations
using the barycentric-differentiable renderer. The soft renderer is able to robustly
optimize larger perturbations than the barycentric renderer.
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--filename_target', type=str, default=os.path.join(data_dir, 'example6b_target.png'))
    parser.add_argument('-o', '--filename_output', type=str, default=os.path.join(data_dir, 'example6b.mp4'))
    ...
    ```
    target_euler_angles = torch.tensor([[0.0, 0.0, 0.0]])
    ```
    """
